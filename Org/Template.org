...
#+begin_src emacs-lisp
  (defun get-parsed-tree ()
    "Use the Org file in the present working directory to obtain a parsed tree."
    (with-current-buffer (find-file-noselect "*.org" nil nil t)
...
It checkes whether the Org file has a #+bucket: keyword.  If it does, then it
simply returns nil.  If it does not, then it adds that bucket name as a keyword
...
#+name:add-bucket-fn
#+header: :var get-tree=get-parsed-tree()
#+begin_src emacs-lisp
