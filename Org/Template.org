...
check:
	  ...
	  @[[ "$(shell $(EDITOR) --eval '(member (quote texinfo) org-export-backends)')" = "(texinfo)" ]] && \
		printf "$${CYAN}Texinfo backend:$${CLEAR} $${GREEN}INSTALLED.$${CLEAR}\n" || \
	  ...
	  @[[ $(shell $(EDITOR) --eval '(symbol-value org-confirm-babel-evaluate)') == "t" ]] && \
		...
		printf "$${CYAN}org-confirm-babel-evaluate:$${CLEAR} $${GREEN}NIL.$${CLEAR}\n\n"
...
texi: $(TEXI)
$(TEXI): $(ORG)
	  $(EDITOR) -u --eval '(with-current-buffer (find-file-noselect "$(ORG)")(org-texinfo-export-to-texinfo))'
