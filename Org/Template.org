...
#+header: :tangle /Users/minilolh/Dev/bin/run-org-template
...
#+begin_src sh -n
  ...
  # verify command-line args contain only alphabets and digits and underscores and spaces
...
#+begin_src emacs-lisp
  (with-current-buffer (car (find-file-noselect "./*.org" nil nil t))
    (goto-char (point-min))
    (let ((re-search-str "\\(?::tangle\\|load-file \\\\[\"]\\)\s*\\(.*?/[dD]ev\\)/")u
	  (dev (getenv "DEV")))
      (while
