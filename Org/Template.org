...
#+author:AUTHOR
#+date:2020-02-19 15:22
#+macro:version Version 0.2.23
...
#+begin_src makefile
  ...
  # This is a target-specific variable for updating the “description” key
  # on the GitHub repo page with the current version number.  It first
  # makes a curl call to the GitHub project repo, finds the “description”
  # line, strips the value of embedded quotes and the prior version number,
  # and finally prints the value with the current version number.  This
  # value is used by the “homepage:” target in the PATCH call.
  homepage: description = $(shell curl -s \
		 -H "Authorization: token $(GH_TOKEN)" \
		 https://api.github.com/repos/$(USER)/$(PROJ)$S | \
		 perl -ne 'if (s/^\s*\"description\": //) { \
		   s/[",\/]|(?:\s*v\s*(?:\d+\.?)+)//g; \
		   print "$$_: $(VERS)"; }')
