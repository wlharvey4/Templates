...

** Bucket Index HTML
The S3 bucket will contain an ~index.html~ file containing links to each Org
entry.  This code checks that:
1. the ~index.html~ file exists;
2. if it does not, then it creates it with this as its first entry;
3. if it does, then it checks that this entry is included;
4. if it is not, then it adds it.
...
#+header: :results output list
#+header: :var bucket=bucket-s3-url()
#+begin_src sh
projs=$(aws s3 ls ${bucket})
declare -a projects
projects=$(for project in $projs; do [ ! $project = "PRE" ] && echo $project; done;)
echo "${projects[@]}"
#+end_src
...
#+begin_src emacs-lisp
(princ (format "<a href=\"%s/%s/index.html\">%s</a>" b-url "mastering_emacs-v0.0.107" "mastering_emacs-v0.0.107"))
#+end_src

#+RESULTS: bucket-anchor
#+begin_export html
<a href="https://pinecone-forest.com/mastering_emacs-v0.0.107/index.html">mastering_emacs-v0.0.107</a>
#+end_export
