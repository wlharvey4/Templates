...
placed at the root of the bucket, ~https://bucket-name/~, and the bucket must
be set up to serve this ~index.html~ when the user hits the root.
...
for project in $projs; do [ ! $project = "PRE" ] && echo $project; done;
#+end_src
...
(progn 
  (princ "<ul>
")
  (let ((p (split-string projects)))
    (seq-do (lambda (u) (princ (format "<li><a href=\"%s/%sindex.html\">%s</a></li>
" b-url u u))) p))
  (princ "</ul>"))
#+end_src

#+RESULTS: bucket-project-links
#+begin_export html
<ul>
<li><a href="https://pinecone-forest.com/aws-guide-v0.0.64/index.html">aws-guide-v0.0.64/</a></li>
<li><a href="https://pinecone-forest.com/clojure-v0.0.29/index.html">clojure-v0.0.29/</a></li>
<li><a href="https://pinecone-forest.com/mastering_emacs-v0.0.107/index.html">mastering_emacs-v0.0.107/</a></li>
<li><a href="https://pinecone-forest.com/mastering_org-v0.8.9/index.html">mastering_org-v0.8.9/</a></li>
<li><a href="https://pinecone-forest.com/scheme-v0.0.4/index.html">scheme-v0.0.4/</a></li>
<li><a href="https://pinecone-forest.com/template-v0.3.34/index.html">template-v0.3.34/</a></li>
</ul>
#+end_export
...
#+name:create-script
#+header: :mkdirp yes
#+header: :tangle /Users/pine/Dev/bin/org-template
#+header: :shebang "#!/usr/bin/env bash"
#+begin_src sh -n
  # $1 :=: Title
  [[ ($# -ge 1) && ($# -le 3) ]] || {
...
#+header: :mkdirp yes
#+header: :shebang "#!/usr/bin/env bash"
#+header: :tangle /Users/pine/Dev/bin/switch-emacs-init
#+begin_src sh
  printf "${GREEN}"
...
#+header: :tangle /Users/pine/Dev/bin/syncrc
#+header: :shebang "#!/usr/bin/env bash"
#+begin_src shell
  [ -v TEMPLATES ] || {
...
#+name:print-bucket
#+header: :results output
#+header: :var bucket-for-print=get-bucket()
#+begin_src emacs-lisp
(princ bucket-for-print)
#+end_src

#+RESULTS: print-bucket
: pinecone-forest

This code calls ~get-bucket~ and returns the value returned as a URL string or
nil.
#+name:bucket-url
#+header: :results value
#+header: :var b=get-bucket()
#+begin_src emacs-lisp
(concat "https://" b ".com")
#+end_src

#+RESULTS: bucket-url
: https://pinecone-forest.com
...
#+RESULTS: bucket-s3-url
: s3://pinecone-forest.com

#+name:bucket-projects
#+header: :results output
#+header: :var bucket=bucket-s3-url()
#+begin_src sh
projs=$(aws s3 ls ${bucket})
for project in $projs; do [ ! $project = "PRE" ] && echo $project; done;
#+end_src

#+RESULTS: bucket-projects
: aws-guide-v0.0.64/
: clojure-v0.0.29/
: mastering_emacs-v0.0.107/
: mastering_org-v0.8.9/
: scheme-v0.0.4/
: template-v0.3.34/

#+name:all-bucket-projects
#+header: :var projects=bucket-projects()
#+header: :var b-url=bucket-url()
#+header: :results output html
#+begin_src emacs-lisp
(progn 
  (princ "<ul>
...
*** Contents for call_print-bucket() Bucket

#+call: all-bucket-projects()
...
starting Emacs.  A little Makefile would work.  This little program should be
stored on GitHub along with the ~Template.org~ file.  When the source is
extracted to a directory, then running this Makefile as ~make~ extracts the
~preprocess.el~ command, which updates =DEV= and then extracts the full
Makefile.  Because this file is tangled along with the main Makefile, it simply
gets added to the end of the big Makefile as an additional rule.  Now running
~make~ runs the default rule, which is to extract everything, then export to
TEXI, INFO, HTML, and PDF forms.

#+header: :tangle Makefile
#+begin_src makefile
boot:
	emacs -Q $${SYNC_ORG_TEMPLATE} --batch --eval '\
	(progn \
