...
#+author:AUTHOR
#+date:2020-01-31 08:22
#+macro:version Version 0.1.18
...
#+begin_src makefile
  ...
  # the AWS S3 bucket you want to use to store the html source files
  BUCKET  := s3://$(AWS_S3_BUCKET)

  # your aws command; this program uses version 2
  AWS     := aws2
  ...
  ### PROGRAM
  ### NOTE: there can be only one Org file in the directory; so far this has not
  ###       been a problem, but it might be.
  PROJ  := $(basename $(shell ls *.org))

  # DIR is not necessarily the same as PROJ;
  # it is the exported .info name found at #+texinfo_filename:<DIR>.info
  # without its extension, used as the directory name for the html exports;
  # use the lowercased PROJ name if there is no texinfo_filename
  DIR   := $(shell sed -nEe '/^\#\+texinfo_filename:(.*)\.info/ { s//\1/p; q; }' $(shell ls *.org))
  DIR   ?= $(shell echo $(PROJ) | tr "[:upper:]" "[:lower:]")


  # If sync is run after the version number has been updated, then VERS picks up the newly-changed value.
  # VERS used to be staticly imbedded when the Makefile was tangled, but it needs to be dynamic for development.
  VERS      := $(shell sed -Ene '/^\#\+macro:version Version (([[:digit:]]+[[:punct:]]?){3})/ { s//v\1\//p; q; }' $(shell ls *.org))

  # PROJ_LIST contains the list of projects currently uploaded to the S3 bucket; each item contains the name
  # of the project and its current version.
  ...
  #perl -MJSON::PP -e '\
	open IN, \
	  "curl -sH \"Authorization: token $ENV{GITHUB_TOKEN}\"\
	  https://api.github.com/user |"; \
	$/ = ""; \
	my $json = JSON::PP->new->loose->allow_barekey->decode(<IN>); \
	print $json->{login}; \
	close IN;'
  USER      := <<get-github-user-info('login')>>
  #USER2 := $(shell node -pe 'JSON.parse(process.argv[1]).login' "$(curl -sH "Authorization: token ${GITHUB_TOKEN}" https://api.github.com/user)")
  #JSON := $(shell curl -sH "Authorization: token $(TOKEN)" https://api.github.com/user)
  #USER2 := $(shell echo $(JSON) | perl -MJSON::PP -e 'my $$json = JSON::PP->new->allow_barekey->decode(<STDIN>); print "WLHARVEY4";')
  #USER2 := $(shell perl -MJSON::PP -e 'open(IN, "curl -sH \"Authorization: token $(TOKEN)\" https://api.github.com/user|"); my $$json = JSON::PP->new->allow_barekey->decode(<IN>); print "WLHARVEY4";')

  #USER2 := $(shell perl -MJSON::PP -e 'open IN, "curl -sH \"Authorization: token $(TOKEN)\" https://api.github.com/user |"; $$/ = ""; my $$json = JSON::PP->new->allow_barekey->decode(<IN>); print $$json->{login}; close IN;')
  USER2 := $(shell curl -sH "Authorization: token $(TOKEN)" https://api.github.com/user | perl -MJSON::PP -e '$$/ = ""; my $$json = JSON::PP->new->loose->allow_barekey->decode(<STDIN>); print $$json->{login};')
