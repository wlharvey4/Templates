...
# Emacs binary, server, and init file
EMACS	  := $(EMACS)
EDITOR	  := $(EDITOR)
...
check:
	  ...
	  @[[ "$(shell $(EDITOR) --eval '(member (quote texinfo) org-export-backends)')" = "(texinfo)" ]] && \
		printf "Texinfo backend is INSTALLED.\n" || \
		{ printf "Texinfo backend is NOT INSTALLED; it must be installed.\n"; exit 1; }

	  @[[ $(shell $(EDITOR) --eval '(symbol-value org-confirm-babel-evaluate)') == "t" ]] && \
		{ printf "org-confirm-babel-evaluate is T; set to NIL.\n"; exit 1; } || \
		printf "org-confirm-babel-evaluate is NIL.\n"
