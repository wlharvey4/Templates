...
*** INPROGRESS Revert INFO buffer upon new INFO file being generated
...
**** Add a Custom_Id
 ...
 #+begin_src emacs-lisp
   ...
   drawer (creating one if necessary) at headline HL."
     (message "In `add-pv-to-hl' with %s %s %s %s" f hl p v)
     (with-current-buffer (find-file-noselect f)
       (save-excursion
         (goto-char (point-min))
         (let ((found (re-search-forward (concat "^*\\{1,\\}\s*" hl) nil t)))
           (cond ((numberp found) (message "...found %s" hl))
                 ((null found)
                  (message "...did not find %s" hl)
                  (beginning-of-line 2)
