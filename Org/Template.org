...
#+author:AUTHOR
#+date:2020-03-18 07:39
#+macro:version Version 0.3.10
:BUCKET:
...
#+begin_src shell
  ...
  usage () {
      printf "${GREEN}USAGE: ${YELLOW}syncrc ${BLUE}[${CYAN}-f system|rc -t rc|system${BLUE}]${CLEAR}\n"
      printf "force sync rc files '-f'rom '-t'o\n"
      exit $1
  }

  touchup () {
      case $from in
	  "system") printf "${BLUE}Touching $1${CLEAR}" ;;
	  "rc")     printf "${BLUE}Touching $2${CLEAR}" ;;
      esac
  }
  ...
  comp () {
		...
		[[ $1 -nt $2 ]] && {
			...
			[[ "$2" -nt "$1" ]] && {
				[[ -w "$1" ]] && {
				    printf "${PURPLE}";
				    #cp -pv "$2" "$1";
				    printf "Copying $2 to $1\n"
				    printf "${CLEAR}"
				} || {
					printf "${RED}You must authenticate... ${CLEAR}"
					printf "${PURPLE}";
					#sudo cp -pv "$2" "$1";
					printf "sudo Copying $2 to $1\n"
					printf "${CLEAR}"
				}
	  ...
	  printf "${WHITE}one file does not exist...${CLEAR}\n"
      }
  }
  ...
  do
      case $opt in
	  'f') printf "Found ${GREEN}$opt: ${CYAN}${OPTARG}${CLEAR}\n"; from=${OPTARG}; ;;&
	  't') printf "Found ${YELLOW}$opt: ${MAGENTA}${OPTARG}${CLEAR}\n";to=${OPTARG}; ;;&
	  'h') usage 0; ;;
  ...
  do
      ...
      case $file in
	    ,*etc?profile)   comp $file $TEMPLATES/rc/etc/profile   ;;
	    ,*etc?bashrc)    comp $file $TEMPLATES/rc/etc/bashrc    ;;
	    ,*etc?tmux.conf) comp $file $TEMPLATES/rc/etc/tmux.conf ;;
	    ,*bash_profile)  comp $file $TEMPLATES/rc/bash_profile  ;;
	    ,*bashrc)        comp $file $TEMPLATES/rc/bashrc        ;;
	    ,*tmux?conf)     comp $file $TEMPLATES/rc/tmux.conf     ;;
      esac
  done
#+end_src
