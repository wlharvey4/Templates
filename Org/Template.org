...
#+begin_src shell
  ...
  comp () {
      ...
      then
		[[ $1 -nt $2 ]] && {
			...
			printf "${BLUE}";cp -pv "$1" "$2";printf "${CLEAR}"
			...
			[[ "$2" -nt "$1" ]] && {
				[[ -w "$1" ]] && {
					printf "${PURPLE}";cp -pv "$2" "$1";printf "${CLEAR}"
				} || {
					printf "${RED}You must authenticate... ${CLEAR}"
					printf "${PURPLE}";sudo cp -pv "$2" "$1";printf "${CLEAR}"
				}
  ...
  for file in /private/etc/profile /private/etc/bashrc /private/etc/tmux.conf ~/.bash_profile ~/.bashrc ~/.tmux.conf
  do
