...
#+name:get-parsed-tree
#+begin_src emacs-lisp
(defun get-parsed-tree ()
  "Return a parsed tree for the Org file in the present working directory, or nil if non exists."
  ;; A necessary condition is that there should be only one Org file in a directory
  (let ((buf (car-safe (find-file-noselect "*.org" nil nil t))))
    ;; buf will be nil if there is no Org file
    (if buf
	(with-current-buffer buf
	  ;; strangely a non-org buffer still parses so check that buf is in org-mode
	  (if (string= "org-mode" (symbol-value (quote major-mode)))
	      (let ((tree (org-element-parse-buffer)))
		(and
		 (message (format "Returning parsed tree for file %s" (buffer-file-name)))
		 tree))
	    (and
	     ;; show this message when buf is not in org-mode
	     (message (format "%s is not an Org file" (buffer-file-name)))
	     nil)))
      ;; show this message when buf is nil
      (and (message (format "No Org file in the Present Working %s" (pwd)))
	   nil))))
#+end_src
...
(defun get-parsed-tree ()
  "Return a parsed tree for the Org file in the present working directory, or nil if non exists."
  ;; There should be only one Org file in a directory
  (let ((buf (car-safe (find-file-noselect "*.org" nil nil t))))
    ;; buf will be nil if there is no Org file
    (if buf
	(with-current-buffer buf
...
(cd "~/Dev/Templates/Org")
"/Users/pine/Dev/Templates/Org/"
...
(defun org-tree (org-dir)
  "Given a directory with an Org template file, return the parsed tree, or nil if none found."
  (and
    (cd org-dir)
    (let ((buf (car-safe (find-file-noselect "*.org" nil nil t))))
      (if buf
