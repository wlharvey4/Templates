...
#+begin_src emacs-lisp
  (setq re (concat (regexp-quote dir) "-\\(v\\(?:[[:digit:]]+\\.?\\)\\{3\\}\\)\\/"))
  (setq str
	(shell-command-to-string
	 (concat
	  "aws2 s3 ls s3://"
	  (substitute-in-file-name "$AWS_S3_BUCKET"))))

  (if (string= file_version "v0.0.0/")
      nil
    (progn
      (string-match re str)
      (princ (match-string-no-properties 1 str))))
#+end_src
